...
def _():
    try:
        ...
        # items = db.execute("SELECT * FROM items WHERE item_is_blocked = 0").fetchall()
            ...
            items = db.execute("SELECT * FROM items WHERE item_is_blocked = 0 ORDER BY item_created_at LIMIT ?", (x.ITEMS_PER_PAGE,)).fetchall()

        items_json = json.dumps(items) # This data will be used in script

        return template("index", user = user, mapbox_token=credentials.mapbox_token, items = items, items_json = items_json, page_number = 1, ITEMS_PER_PAGE = x.ITEMS_PER_PAGE)
