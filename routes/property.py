...
def _():
    try:
        ...
        if user['user_role'] == "admin":
            ...
            q = db.execute("SELECT * FROM items WHERE owner_id = ? ORDER BY item_created_at LIMIT 0, ?", (user["user_pk"], x.ITEMS_PER_PAGE,))
        items = q.fetchall()
...
def _(page_number):    
    try:
        ...
        btn_more = template("__btn_more", page_number=next_page, user = user)
